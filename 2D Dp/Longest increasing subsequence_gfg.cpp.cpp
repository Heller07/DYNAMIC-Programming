//Recursion
class Solution
{
    public:
    //Function to find length of longest increasing subsequence.
    int solve(int n,int a[],int curr,int prev){
        //base case
        if(curr==n){
            return 0;
        }
        //take or include
        int include = 0;//basically it is a count of including numbers
        if(prev == -1 || a[curr]>a[prev]){
            include = 1+solve(n,a,curr+1,curr);
        }
        //exclude
        int exclude = 0 + solve(n,a,curr+1,prev);
        return max(include,exclude);
         
    }
    int longestSubsequence(int n, int a[])
    {
       // your code here
       return solve(n,a,0,-1);
    }
};
//TOP DOWN
class Solution
{
    public:
    //Function to find length of longest increasing subsequence.
    int solve(int n,int a[],int curr,int prev,vector<vector<int>>&dp){
        //base case
        if(curr==n){
            return 0;
        }
        if(dp[curr][prev+1] != -1){
            return dp[curr][prev+1];
        }
        //take or include
        int include = 0;//basically it is a count of including numbers
        if(prev == -1 || a[curr]>a[prev]){
            include = 1+solve(n,a,curr+1,curr,dp);
        }
        //exclude
        int exclude = 0 + solve(n,a,curr+1,prev,dp);
        dp[curr][prev+1] = max(include,exclude);
        return dp[curr][prev+1];//for saving invalid index
         
    }
    int longestSubsequence(int n, int a[])
    {
       // your code here
       vector<vector<int>> dp(n,vector<int>(n+1,-1));
       return solve(n,a,0,-1,dp);
    }
};
//BOTTOM UP
class Solution
{
    public:
    //Function to find length of longest increasing subsequence.
    int solve(int n,int a[]){
        //base case
        vector<vector<int>> dp(n+1,vector<int>(n+1,0));
        for(int curr = n-1;curr>=0;curr--){
            for(int prev = curr-1;prev>=-1;prev--){
                int include = 0;//basically it is a count of including numbers
                if(prev == -1 || a[curr]>a[prev]){
                include = 1+dp[curr+1][curr+1];
                    }
        //exclude
                 int exclude = 0 + dp[curr+1][prev+1];
                dp[curr][prev+1] = max(include,exclude);
            }
        }
                 return dp[0][0];//for saving invalid index
    }
    int longestSubsequence(int n, int a[])
    {
       // your code here
       return solve(n,a);
    }
};
//SP
class Solution
{
    public:
    //Function to find length of longest increasing subsequence.
    int solve(int n,int a[]){
        //base case
        vector<int> currrow(n+1,0);
        vector<int> nextrow(n+1,0);
        for(int curr = n-1;curr>=0;curr--){
            for(int prev = curr-1;prev>=-1;prev--){
                int include = 0;//basically it is a count of including numbers
                if(prev == -1 || a[curr]>a[prev]){
                include = 1+nextrow[curr+1];
                    }
        //exclude
                 int exclude = 0 + nextrow[prev+1];
                currrow[prev+1] = max(include,exclude);
            }
            nextrow = currrow;
        }
                 return nextrow[0];//for saving invalid index
    }
    int longestSubsequence(int n, int a[])
    {
       // your code here
       return solve(n,a);
    }
};
//MORE OPTIMISED WITH DP WITH BInARY SEARCH
class Solution
{
    public:
    //Function to find length of longest increasing subsequence.
    int solve(int n,int a[]){
        if(n==0){
            return 0;
        }
        vector<int> ans;
        ans.push_back(a[0]);
        for(int i = 1;i<n;i++){
            if(a[i]>ans.back()){
                ans.push_back(a[i]);
            }
            else{
                //find index of just bada element in ans
                int index = lower_bound(ans.begin(),ans.end(),a[i])-ans.begin();
                ans[index] = a[i];
            }
        }
        return ans.size();
    }
        
    
    int longestSubsequence(int n, int a[])
    {
       // your code here
       return solve(n,a);
    }//SC IS O(N)
